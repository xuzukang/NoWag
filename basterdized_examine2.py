import yaml
import os
original_yaml_path = "/data/lliu/huffman/models/meta-llama/Llama-2-7b-hf/compressed/desert-pyramid-109/checkpoints.yaml"
...
keys = sorted(list(original_yaml.keys()))

for k in keys:
    print(k)
    small_dict = {k: original_yaml[k]}
    model = load_model_from_checkpoints(small_dict,
                                        # lambda x: "joint2" if "self_attn" in x else "quantize",
                                        base_model,
                                        model,
    ...
    print("ppl:", llama_eval(model, testloader, "cuda:1", dataset, False,
                             base_model = base_model,
                             disable_tqdm = True))
