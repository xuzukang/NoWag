...
class LowRankLinear(CompressedLinear):
    ...
    def compress(self, rank:int,
                 normalizer_kwargs:dict = {},
                normalizer:Normalizer = None):
        return self.low_rank(rank=rank, normalizer_kwargs=normalizer_kwargs, normalizer=normalizer)
    
    def blank_recreate(self, rank, normalizer_kwargs:dict = {}, normalizer:Normalizer = None):
        if normalizer is not None:
            self.normalizer = normalizer

        else:
            self.normalizer = Normalizer.blank_recreate(self.original_weight, **normalizer_kwargs)
    ...
    def _no_checkpoint_forward(self, x):
        ...
        if self.forward_method == "oft":
            if self.denormalization_method == "otf":
                return self.normalizer.denormalize_otf_out(F.linear(F.linear(self.normalizer.denormalize_otf_in(x), self.B), self.A, self.bias))
            elif self.denormalization_method == "ignore":
                return F.linear(F.linear(x, self.B), self.A, self.bias)
            else:
                raise NotImplementedError(f"denormalization method {self.denormalization_method} not implemented")
            
        else:
            if self.denormalization_method == "otf":
                return self.normalizer.denormalize_otf_out(F.linear(x, self.reconstruct(denormalize = False), self.bias))
            elif self.denormalization_method == "ignore":
...
if __name__ == "__main__":
    ...
    low_rank.low_rank(rank=256, normalizer_kwargs= dict(norm_order=[0,1], zero=[False, False]))
