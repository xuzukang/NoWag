...
def magnitude_unstructured_sparsity(weight:torch.FloatTensor, hessian:torch.FloatTensor, sparsity:float):
...
def magnitude2_row_column_sparsity(weight:torch.FloatTensor, hessian:torch.FloatTensor, sparsity:float):
    """Magnitude based row-column sparsity
    """
    norm_0 = torch.norm(weight, p=2, dim=0)
    norm_1 = torch.norm(weight, p=2, dim=1)

    threshold = torch.kthvalue(torch.concat([norm_0, norm_1]), int(sparsity * weight.numel())).values
