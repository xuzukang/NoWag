...
class ScalarQuantizer(quantizer_parent.QuantizerParent):
    def __init__(
        ...
        if sparse_mask is not None:
            ...
            assert (
                sparse_values is not None
            ), "sparse_values must be provided if sparse_mask is provided"
    ...
    def update_discrete(self):
        ...
        ), "reference_weight must be provided to update the discrete values"
        with torch.no_grad():
            self.codes = quantizer_utils.round_to_the_nearest(
                self.get_reference_weight().view(-1), self.codebook
            )
    ...
    def get_n_bits(self):
        ...
        if self.sparse_mask is not None:
            n_bits += (
                self.sparse_mask.numel() * 3 * 16
            )  # 3 because we need 4 bytes for the location of this sparse value
