...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_model",
        type=str,
        help="the hf base model to use",
        default="meta-llama/Llama-2-7b-hf",
    )
    parser.add_argument(
        "--quantized_weight_yaml",
        type=str,
        help="the path to the quantized weight",
        default=None,
    )
    parser.add_argument(
        "--quantized_weight_hf",
    ...
    parser.add_argument(
        "--device",
        type=str,
        help="the device to run the compression on",
        default="cuda:0",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["winogrande", "piqa", "hellaswag", "arc_easy", "arc_challenge"],
    )
    ...
    parser.add_argument(
        "--save", help="Save the results to the specified path", action="store_true"
    )
    ...
    if args.quantized_weight_yaml is not None:
        model = get_llama(args.base_model, device_map="balanced", dtype=torch.float16)
        checkpoints_paths = yaml.load(
            open(args.quantized_weight_yaml, "r"), Loader=yaml.FullLoader
        )

        model, n_bits, n_params = load_model_from_checkpoints(
        ...
        utils.recursive_apply(
            model, "cache_reconstruct", {"denormalize": True, "offload": True}
        )
    else:
        print("no compressed model was provided")
        model = get_llama(args.base_model, device_map="auto", dtype=torch.float16)
    ...
    if args.save:
        if args.output_path is None:
            if args.quantized_weight_yaml is not None:
                args.output_path = (
                    os.path.dirname(args.quantized_weight_yaml) + "/eval_results.yaml"
                )
            elif args.quantized_weight_hf is not None:
                args.output_path = (
                    os.path.dirname(
                        args.quantized_weight_hf.replace("compressed_hf", "compressed")
                    )
                    + "/eval_results.yaml"
                )
            else:
        ...
        if os.path.exists(args.output_path):
            existing_results = yaml.load(
                open(args.output_path, "r"), Loader=yaml.FullLoader
            )
        else:
            existing_results = {}
