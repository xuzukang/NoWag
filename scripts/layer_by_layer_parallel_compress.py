...
parser.add_argument(
    "--seqlens", type=int, nargs="+", default=[4096], help="list of seqlens to compress"
)
parser.add_argument(
    "--batch_size",
    type=int,
...
parser.add_argument(
    "--hessian_path",
    type=str,
    default="/data/lliu/huffman/models/{model_name}/hessians_new/pajama/128/",
...
parser.add_argument(
    "--weights_path",
    type=str,
    default="/data/lliu/huffman/models/{model_name}/original_weights",
)
parser.add_argument(
    "--save_path",
    type=str,
    default="/data/lliu/huffman/models/{model_name}/compressed",
    help="path to save the compressed models",
)
parser.add_argument(
    "--self_attn_compression_algorithm",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    default=["cuda:5", "cuda:6", "cuda:4", "cuda:7"],
...
parser.add_argument(
    "--yaml_path",
    type=str,
    default="/data/lliu/huffman/scripts/1layer_compress/quantizer_args.yaml",
)
...
parser.add_argument(
    "--use_already_done",
    action="store_true",
    help="if provided will overwrite the save path",
)
parser.add_argument(
    "--use_wandb", action="store_true", help="if provided will use wandb"
)
parser.add_argument(
    "--resume_wandb", action="store_true", help="if provided will resume the wandb run"
)
...
parser.add_argument(
    "--no_config_update",
    action="store_true",
    help="if provided will not update the wandb config",
)
parser.add_argument(
    "--ppl_eval", action="store_true", help="if provided will run perplexity eval"
)
...
if args.use_wandb:
    ...
    if not args.no_config_update:
        ...
        if args.self_attn_yaml_path is not None:
            config["self_attn_args"] = yaml.load(
                open(args.self_attn_yaml_path, "r"), Loader=yaml.FullLoader
            )
        else:
            config["self_attn_args"] = yaml.load(
                open(args.yaml_path, "r"), Loader=yaml.FullLoader
            )
        if args.mlp_yaml_path is not None:
            config["mlp_args"] = yaml.load(
                open(args.mlp_yaml_path, "r"), Loader=yaml.FullLoader
            )
        else:
            config["mlp_args"] = yaml.load(
                open(args.yaml_path, "r"), Loader=yaml.FullLoader
    ...
    if args.run_name is None:
        n_runs = glob.glob(
            args.save_path.replace("{model_name}", args.models_to_compress[0]) + "/*"
        )
...
if args.yaml_path is not None:
    yaml.dump(
        yaml.load(open(args.yaml_path, "r"), Loader=yaml.FullLoader),
        open(os.path.join(temp_yaml_path, "yaml.yaml"), "w"),
    )
    args.yaml_path = os.path.join(temp_yaml_path, "yaml.yaml")
if args.self_attn_yaml_path is not None:
    yaml.dump(
        yaml.load(open(args.self_attn_yaml_path, "r"), Loader=yaml.FullLoader),
        open(os.path.join(temp_yaml_path, "self_attn.yaml"), "w"),
    )
    args.self_attn_yaml_path = os.path.join(temp_yaml_path, "self_attn.yaml")
if args.mlp_yaml_path is not None:
    yaml.dump(
        yaml.load(open(args.mlp_yaml_path, "r"), Loader=yaml.FullLoader),
        open(os.path.join(temp_yaml_path, "mlp.yaml"), "w"),
    )
...
for model_name in args.models_to_compress:
    model_paths = glob.glob(
        args.hessian_path.format(model_name=model_name) + "/**/*.pt", recursive=True
    )
...
seqlen_map = {
    model_name: args.seqlens[i] for i, model_name in enumerate(args.models_to_compress)
}
...
def run_command(command: str, device: str, log_path: str, command_name: str):
    global DEVICES_DICT
...
def read_log(log_path: str, wandb_log_prefix: str = "") -> bool:
    ...
    try:
        ...
        print(
            "best_loss", best_loss, "running bpv:", round(TOTAL_BITS / TOTAL_PARAMS, 6)
        )
        if args.use_wandb:
...
def check_still_running(devices_dict) -> list[str]:
    ...
    for device, out in devices_dict.items():
        ...
        if check_pid(pid):
            ...
            DONE_SAVE_PATHS[log_path_map[log_path]][command_name] = log_path.replace(
                "compressed.log", "compressed.pt"
            )
            DEVICES_OPEN.append(device)
            COMMANDS_FINISHED += 1
    ...
    for key in dict_reference.keys():
        ...
        if key not in dict_to_check.keys():
            ...
            if not check_dict(
                ...
                (
                    ["d", "n_bits"]
                    if key == "quantizer_kwargs"
                    and "allocation_config" in dict_to_check.keys()
                    else []
                ),
            ):
                return False
...
def make_command(load_path: str, model_name: str) -> tuple[str, str, str]:
    ...
    command_name = (
        ...
        + load_path.split("/")[-2]
        + "/"
        + load_path.split("/")[-1][: load_path.split("/")[-1].rfind(".")]
    )
    ...
    if "mlp" in load_path:
        ...
        yaml_path = (
            args.mlp_yaml_path if args.mlp_yaml_path is not None else args.yaml_path
        )
    elif "self_attn" in load_path:
        compression_algorithm = args.self_attn_compression_algorithm
        yaml_path = (
            args.self_attn_yaml_path
            if args.self_attn_yaml_path is not None
            else args.yaml_path
        )
    ...
    save_path = os.path.join(
        args.save_path.replace("{model_name}", model_name),
        command_name,
        "compressed.pt",
    )
    ...
    if args.discrete_update_hessian_path is not None:
        discrete_update_hessian_path = args.discrete_update_hessian_path.replace(
    ...
    if args.use_already_done:
        ...
        if "None" not in args.already_done_runs:
            ...
            prev_run_names += [
                path.split("/")[model_name_idx]
                for path in glob.glob(args.save_path.replace("{model_name}", "*"))
            ]
        ...
        for prev_run_name in prev_run_names:
            for path in glob.glob(
                save_path.replace(run_name, prev_run_name).replace(
                    "compressed.pt", "compressed_args.yaml"
                )
            ):
                print("path", path)
                ...
                if is_same:
                    ...
                    print(
                        "loading from",
                        path.replace("compressed_args.yaml", "compressed.pt"),
                    )
                    if read_log(
                        path.replace("compressed_args.yaml", "compressed.log"),
                        wandb_log_prefix=command_name + "/",
                    ):
                        print("already done with ", command_name)
                        # raise ValueError("stop here")
                        return (
...
for path in paths:
    ...
    if command is not None:
        ...
        DONE_SAVE_PATHS[inverse_path_map[path]][command_name] = log_path.replace(
            "compressed.log", "compressed.pt"
        )
...
while COMMANDS_FINISHED < n_commands:
    ...
    for key in DONE_SAVE_PATHS.keys():
        ...
        if len(DONE_SAVE_PATHS[key]) == len(path_map[key]):
            ...
            checkpoint_list_path = os.path.join(
                args.save_path.replace("{model_name}", key), "checkpoints.yaml"
            )
            ...
            if os.path.exists(checkpoint_list_path):
                prev_checkpoint_list = yaml.load(
                    open(checkpoint_list_path, "r"), Loader=yaml.FullLoader
                )
            ...
            if args.ppl_eval:
                ...
                log_paths.insert(
                    0,
                    os.path.join(
                        args.save_path.replace("{model_name}", key), "ppl_eval.log"
                    ),
                )
                n_commands += 1
