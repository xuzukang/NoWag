...
def sparse_layer(
    ...
    for name in sublayer_names:
        ...
        original_layer.weight.data = (
            sparse_layer.reconstruct()
            .to(original_devices_dtypes[name][1])
            .to(original_devices_dtypes[name][0])
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sparse_kwargs_path", type=str, help="the path to the sparse kwargs"
    )
    parser.add_argument(
        "--calibration_dataset",
        type=str,
        help="the calibration dataset",
    ...
    parser.add_argument(
        "--save_and_load_temp_path", type=str, default="temp/temp_model"
    )
    ...
    model = sparse_model(
        model,
        dataloader,
        sparse_kwargs=yaml.load(
            open(args.sparse_kwargs_path, "r"), Loader=yaml.FullLoader
        )["sparse_kwargs"],
        device=args.device,
    ...
    for dataset in args.ppl_datasets:
        ...
        ppl = ppl_eval.llama_eval(
            model,
            testloader,
            args.device,
            dataset,
            args.log_wandb,
            args.offload_activations,
            args.batch_size,
    ...
    if args.save_and_load_model:
        ...
        model = ppl_eval.get_llama(
            args.base_model, model_path=args.save_and_load_temp_path, device_map="auto"
        )
        model.seqlen = args.seqlen
        model.eval()

    if "None" not in args.zero_shot_tasks:
        results["zero_shot"] = zs.zero_shot(
            args.base_model, model, device=args.device, tasks=args.zero_shot_tasks
        )
