...
def weighted_kde_entropy(
    ...
    if x_range is None:
        ...
        x_range = (
            overall_range[0] - multiplier * bandwidth,
            overall_range[1] + multiplier * bandwidth,
        )
    ...
    if mask is None:
        ...
        for i in range(samples.shape[0]):
            estimated_pdf *= i / (i + 1)
            estimated_pdf += torch.sum(
                gaussian_pdf(x.unsqueeze(-1), samples[i].unsqueeze(0), bandwidth)
                * normalized_weights.unsqueeze(0),
                dim=1,
            ) / (i + 1)
    else:
        n_sum = 0
        for i in range(samples.shape[0]):
            ...
            estimated_pdf += torch.sum(
                gaussian_pdf(
                    x.unsqueeze(-1), samples[i, row_mask].unsqueeze(0), bandwidth
                )
                * weights_mask.unsqueeze(0),
                dim=1,
...
parser.add_argument(
    "--target_sparse_frac", type=float, default=0.0, help="The target sparsity fraction"
)
parser.add_argument(
    "--layer_max_sparse_frac",
    type=float,
    default=0.01,
    help="The maximum sparsity fraction for each layer",
)
parser.add_argument(
    "--sparsity_structure",
    type=str,
    choices=["unstructured", "block", "row", "column"],
...
parser.add_argument(
    "--target_bpv", type=int, default=2, help="The target number of bits"
)
parser.add_argument(
    "--possible_bpv",
    type=float,
    default=[1, 2, 3, 4],
    nargs="+",
    help="The possible number of bits",
)

parser.add_argument(
    "--output_path",
    type=str,
    default="/data/lliu/huffman/models/{model_name}/allocation",
)
...
normalizer_kwargs = {
    "norm_order": args.normalizer_norm_order,
...
with torch.no_grad():
    for w_path in tqdm.tqdm(weights_path):
        ...
        w = torch.load(w_path, weights_only=True, map_location=device)["weight"].to(
            torch.float32
        )
        _, w = normalizer.Normalizer.normalize_init(w, **normalizer_kwargs)

        diag_weigths = torch.diag(
            torch.load(
                w_path.replace(weight_dir, hessians_dir),
                weights_only=True,
                map_location=device,
            )["hessian"]
        ).to(torch.float32)
        ...
        if args.wanda_entropy:
            if name not in entropies_dict:
                entropies_dict[name] = weighted_kde_entropy(
                    importances_wanda[name]
                ).item()
        else:
            if name not in entropies_dict:
...
if args.target_sparse_frac > 0:
    n_sparse_target = args.target_sparse_frac * sum(
        n_numel_dict.values()
    )  # the total number of sparse values
    running_sparse = 0
    ...
    if args.layer_max_sparse_frac <= args.target_sparse_frac:
        ...
        for key in n_numel_dict.keys():
            ...
            if args.sparsity_structure == "unstructured":
                ...
                mask = sparse.UnstructuredSparse.generate_mask(
                    importances, n_sparse_layer
                )

                if args.wanda_entropy:
                    entropies_dict[key] = weighted_kde_entropy(
                        importances, mask=mask
                    ).item()
                else:
                    entropies_dict[key] = weighted_kde_entropy(
                        importances, hessian_weights[key], mask=mask
                    ).item()
        ...
        while running_sparse < n_sparse_target:
            sorted_keys = np.argsort(
                [entropies_dict[key] for key in entropies_dict.keys()]
            )[::-1]
            ...
            for key in sorted_keys:
                ...
                if args.sparsity_structure == "unstructured":
                    ...
                    if (
                        layer_sparsity[key]
                        >= args.layer_max_sparse_frac * n_numel_dict[key]
                    ):
                        continue
                    ...
                    importances = importances_wanda[key]
                    mask = sparse.UnstructuredSparse.generate_mask(
                        importances, int(n_sparse_layer)
                    )
...
if len(args.possible_bpv) == 1:
    assert (
        args.possible_bpv[0] == args.target_bpv
    ), "If only one possible bpv is given, it should be the target bpv"
    ...
    constraints = [
        cp.sum(bpv_layer_cont * n_values) <= target_b_bits,
        bpv_layer_cont >= 0,
    ]
    ...
    print(
        "n_bits",
        np.sum(bpv_layer_cont * n_values),
        "bpv",
        np.sum(bpv_layer_cont * n_values) / sum(n_values),
    )

    possible_bpv = np.array(args.possible_bpv)

    # round the bits per value to the nearest possible value
    bpv_discrete = np.array(
        [possible_bpv[np.argmin(np.abs(possible_bpv - bpv))] for bpv in bpv_layer_cont]
    )
        ...
        while n_bits > target_b_bits:
            ...
            for i in layer_idx:
                ...
                if n_bits > target_b_bits:
                    ...
                    bpv_discrete[i] = np.max(
                        possible_bpv[possible_bpv < bpv_discrete[i]]
                    )
    ...
    print(
        "post allocation distortion", np.sum(np.exp(2 * entropies - 2 * bpv_discrete))
    )
...
for i, key in enumerate(entropies_dict.keys()):
    allocation_dict[key[1:]] = {
        "n_bits": float(bpv_discrete[i]),
        "sparse_frac": float(layer_sparsity[key] / n_numel_dict[key]),
    }
