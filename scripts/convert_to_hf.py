...
def main():
    ...
    orig_config = AutoConfig.from_pretrained(
        base_model, dtype="auto", device_map="cpu", attn_implementation="sdpa"
    )
    ...
    compression_kwargs = yaml.load(
        open(
            (checkpoints_dict[list(checkpoints_dict.keys())[0]]).replace(
    ...
    for checkpoint in checkpoints_dict.values():
        assert compression_kwargs == yaml.load(
            open(checkpoint.replace("compressed.pt", "compressed_args.yaml"), "r"),
            Loader=yaml.FullLoader,
        )
    ...
    for checkpoint_name, checkpoint_path in checkpoints_dict.items():
        ...
        layer = getattr(
            getattr(model.model.layers[i_layer], submodule_name), linear_name
        )
