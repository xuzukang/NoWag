...
class Config:
    ...
    optimizer_config: Optional[OmegaConf] = field(default_factory=lambda: {"_target_": "torch.optim.AdamW", "lr": 1e-5})
...
def get_loss(layer:llama.LlamaDecoderLayer,
...
def train_layer(layer:llama.LlamaDecoderLayer,
    ...
    with torch.no_grad():
        val_loss = 0
        n_iter = 0
        for i in tqdm.tqdm(range(0, len(valid_inps), train_config.batch_size_val), desc = f"Validating Epoch -1",leave = False, disable=True):
    ...
    torch.save(layer.state_dict(), os.path.join(train_config.temp_dir, f"{prefix}_best.pt"))
    
    for epoch in tqdm.tqdm(range(train_config.num_epochs), desc = f"Training {prefix}", leave = False):
        #train the layer
        layer.train()
        n_iter = 0
        overall_loss = 0
        for i in tqdm.tqdm(range(0, len(train_inps), train_config.batch_size), desc = f"Epoch {epoch}",leave = False,disable=True):
            ...
            if (n_iter + 1) % train_config.grad_accum_steps == 0:
                ...
                if train_config.clip_grad > 0:
                    norm = nn.utils.clip_grad_norm_(layer.parameters(), train_config.clip_grad)
                #otherwise just record the norm of the gradients
                else:
                    norm = nn.utils.get_total_norm([p.grad for p in layer.parameters() if p.grad is not None])
            ...
            wandb.log({f"{prefix}_train_loss": loss.item(), f"{prefix}_grad_norm": norm})
        ...
        with torch.no_grad():
            val_loss = 0
            n_iter = 0
            for i in tqdm.tqdm(range(0, len(valid_inps), train_config.batch_size_val), desc = f"Validating Epoch {epoch}",leave = False, disable=True):
                inputs = valid_inps[i:i + train_config.batch_size_val].cuda()
                labels = valid_outputs[i:i + train_config.batch_size_val].cuda()
        ...
        if val_loss < best_loss:
            ...
            patience_counter += 1
            tqdm.tqdm.write(f"Patience counter {patience_counter}, allowed {train_config.early_stop_patience}")
            if patience_counter == train_config.early_stop_patience: #this allows for negative patience to disable early stopping
    ...
    if best_loss != val_loss:
        layer.load_state_dict(torch.load(os.path.join(train_config.temp_dir, f"{prefix}_best.pt")))
...
@hydra.main(version_base=None, config_path="../config", config_name="layer_by_layer_ft_config")
def main(cfg: DictConfig):
    ...
    use_cache = quantized_model.config.use_cache
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(overall_data, desc="getting inputs", miniters=len(overall_data)//100):
            try:
    ...
    train_args = instantiate({"_target_": Config},
                             _recursive_=False,
                             **cfg.ft_args)
    ...
    #initiate wandb
    wandb.init(project="llama_layerwise_ft",
    ...
    for i_layer in tqdm.tqdm(range(len(base_model.model.layers))):
        ...
        utils.clean()
        #train the layer
        quantized_model.model.layers[i_layer] = train_layer(quantized_model.model.layers[i_layer],     
        ...
        if cfg.sequential:
            quantized_layer = quantized_model.model.layers[i_layer].cuda()
            outputs = model_utils.inference_layer(
    ...
    #save the model
    quantized_model.save_pretrained(cfg.model.save_path)
    wandb.finish()
