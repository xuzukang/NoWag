...
parser.add_argument(
    "--hessian_path",
    type=str,
    default="/data/lliu/huffman/models/meta-llama/Llama-2-7b-hf/hessianDiags/seed_0/pajama/128/layer_0/self_attn.q_proj.pt",
)
parser.add_argument(
    "--weights_path",
    type=str,
...
parser.add_argument(
    "--save_path", type=str, default="/data/lliu/huffman/test/save_self_attn.q_proj.pt"
)
parser.add_argument(
...
parser.add_argument(
    "--yaml_path",
    type=str,
    default="/data/lliu/huffman/yamls/quantizer/2bits_6d_basic.yaml",
)
...
if kwargs["quantizer_type"] == "LinearVQ":
    compression_module = qc.LinearVQ(weight.to(args.device).to(dtype), verbose=True)
elif kwargs["quantizer_type"] == "LinearVQ_Halving":
    compression_module = qc.LinearVQ_Halving(
        weight.to(args.device).to(dtype), verbose=True
    )
else:
...
if "hessian" in hessian:
    ...
    if kwargs.get("hessian_regularization", 0) > 0:
        ...
        compression_module.hessian += (
            kwargs["hessian_regularization"]
            * torch.eye(compression_module.hessian.size(0)).to(args.device)
            * diag_mean
        )
...
print(
    ...
    compression_module.get_reconstruction_error(
        compression_module.hessian
        if hasattr(compression_module, "hessian")
        else compression_module.hessianDiag
    ).item(),
)
