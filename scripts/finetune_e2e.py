...
class SimpleDataset(Dataset):
    ...
    def __init__(self, inputs, labels, soft_labels: bool = False):
...
def make_datasets(
    X: torch.FloatTensor, Y: torch.FloatTensor, n_val: int, soft_labels: bool
    ...
    train_ds = SimpleDataset(X[train_idxs], Y[train_idxs], soft_labels=soft_labels)
    valid_ds = SimpleDataset(
        X[idxs[-n_val:]], Y[idxs[-n_val:]], soft_labels=soft_labels
    )
...
def calculate_logits(model: llama.LlamaForCausalLM, devset, batch_size):
    ...
    for i in tqdm.tqdm(range(len(devset) // batch_size), desc="Calculating logits"):
        logits.append(
            model(devset[i * batch_size : (i + 1) * batch_size].cuda())["logits"].cpu()
        )
...
@hydra.main(
    version_base=None, config_path="../config", config_name="quantized_ft_config"
)
def main(cfg: DictConfig):
    ...
    if cfg.soft_labels:
        overall_out = calculate_logits(
            base_model, overall_data, dataset_cfg.logits_batch_size
        )
        print(overall_out)
    ...
    quantized_model = llama.LlamaForCausalLM.from_pretrained(
    ...
    if cfg.soft_labels:
        trainset, valset = make_datasets(
            overall_data, overall_out, dataset_cfg.ft_n_val, soft_labels=True
        )
    else:
        trainset, valset = make_datasets(
            overall_data, overall_data, dataset_cfg.ft_n_val
        )
    ...
    optimizer = (
        ...
        else instantiate(cfg.optimizer, params=quantized_model.parameters())
    )
    scheduler = (
        None
        if not hasattr(cfg, "scheduler")
        else instantiate(cfg.scheduler, optimizer=optimizer)
    )
