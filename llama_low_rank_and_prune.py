...
def turn_on_batch_add(module):
    for name, child in module.named_children():
        if isinstance(child, MLP_pruner.pruned_feed_forward):
            child.turn_on_batch_add()
        elif isinstance(child, MHA_low_rank.Low_Rank_linear):
            child.turn_on_batch_add()
        else:
        ...
        for l,names in enumerate(sequential):
            ...
            for name in names:
                if name == "mlp":
                    ...
                    del mlp
                #otherwise if self_attn in the name
                elif "self_attn" in name:
                    self_attn = getattr(layer, name.split(".")[0])
                    setattr(self_attn, name.split(".")[1], 
                            MHA_low_rank.Low_Rank_linear(getattr(self_attn, name.split(".")[1]).weight))
                    getattr(self_attn, name.split(".")[1]).turn_on_batch_add()
            ...
            for name in names:
                ...
                print("Pruning ...")
                if name == "mlp":
                    module:MLP_pruner.pruned_feed_forward = getattr(layer, name)
                    
                    module.prune(
                        args.keep_top_frac,
                        args.keep_bottom_frac,
                        damping=args.percdamp,
                        d = 4
                    )
                    
                    #disable the grad for this layer
                    module.disable_grad()
                
                elif "self_attn" in name:
                    module:MHA_low_rank.Low_Rank_linear = getattr(getattr(layer, name.split(".")[0]), name.split(".")[1])
                    module.low_rank(
                        args.mha_low_rank,
                        args.sparse_rowwise,
                        args.sparse_rowwise_conditions,
                        args.sparse_colwise,
                        args.sparse_colwise_conditions,
                        args.regularization,
                        args.percdamp,
                        align_epochs = args.n_iters,
                        lr = args.lr,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--keep_top_frac", type=float, default=0.5, help="Keep top fraction."
    )
    parser.add_argument(
        "--keep_bottom_frac", type=float, default=0.0, help="Keep bottom fraction."
    )
    parser.add_argument(
        "--mha_low_rank", type=int, default=256, help="Low rank for MHA."
    )
    parser.add_argument(
